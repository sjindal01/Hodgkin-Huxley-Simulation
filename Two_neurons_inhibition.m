...
A1=.5; %controls the strength of inhibition
A2=.5;
...
for i=1:length(t)-1
 ...
 V(i+1)=V(i)+(dt)*((1/Cm)*(I1(i)+stimulus1(i)-(INa+IK+Il)));
 
 V2(i+1)=V2(i)+(dt)*((1/Cm)*(I2(i)+stimulus2(i)-(INa2+IK2+Il2)));
 ...
 stimulus1(i+1)=normrnd(.5,.3);
 stimulus2(i+1)=normrnd(.5,.3);
